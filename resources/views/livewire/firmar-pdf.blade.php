<div class="max-w-xl mx-auto p-6 bg-white/80 dark:bg-zinc-900/80 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 backdrop-blur-xl animate-fade-in">
    ...
    <div class="mb-4 p-3 bg-yellow-100 dark:bg-yellow-900/40 text-yellow-800 dark:text-yellow-200 rounded shadow text-sm">
        <b>Requisito:</b> Debes tener instalado <b>Ghostscript</b> en tu equipo para poder previsualizar los archivos PDF.<br>
    ...
    <form wire:submit.prevent="firmar" class="space-y-6">
        <div>
        ...
        <div>
            <label class="block text-sm font-medium mb-1">Imagen de firma (Formato PNG)</label>
            <input type="file" wire:model="firma" accept="image/png,image/jpeg" class="block w-full text-sm text-gray-700 dark:text-gray-200 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-100 file:text-blue-700 dark:file:bg-blue-900 dark:file:text-blue-200 hover:file:bg-blue-200 dark:hover:file:bg-blue-800 transition" />
            @error('firma')
                <div class="text-red-500 text-sm animate-shake">{{ $message }}</div>
            @enderror
        </div>
        <div>
            <label class="block text-sm font-medium mb-1">Aclaración</label>
            <input type="text" wire:model.defer="aclaracion" maxlength="100"
                class="w-full rounded-lg border-2 border-blue-200 dark:border-blue-700 bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition shadow-sm"
        ...
        <div>
            ...
            <input type="text" wire:model.defer="dni" maxlength="20"
                class="w-full rounded-lg border-2 border-blue-200 dark:border-blue-700 bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition shadow-sm"
        ...
        @if($previewImagePath)
        <div class="mb-4">
            <label class="block text-sm font-medium mb-1">Selecciona una o más posiciones en la cuadrícula:</label>
            <div class="relative w-full max-w-lg mx-auto aspect-[595/842] border border-blue-200 rounded overflow-hidden">
                <img src="{{ $previewImagePath }}" alt="Previsualización PDF" class="w-full h-auto select-none pointer-events-none" />
                <div class="absolute inset-0 grid grid-cols-5 grid-rows-8">
                    @for($y = 1; $y <= 8; $y++)
                        @for($x = 1; $x <= 5; $x++)
                            @php
                                $selected = collect($selectedCells)->contains(fn($cell) => $cell['x'] == $x && $cell['y'] == $y);
                            @endphp
                            <button type="button" wire:click="toggleCell({{ $x }}, {{ $y }})"
                                class="border border-blue-300/30 hover:border-blue-500 focus:border-blue-700 transition-all duration-100 w-full h-full @if($selected) bg-blue-400/30 dark:bg-blue-700/30 ring-2 ring-blue-600/60 @endif">
                                @if($selected)
                                    <span class="block w-3 h-3 mx-auto my-auto rounded-full bg-blue-600"></span>
                                @endif
                            </button>
                        @endfor
                    @endfor
            ...
            <div class="text-xs text-gray-500 mt-2">Puedes seleccionar varias celdas para definir el área donde se ubicará la firma, aclaración y DNI.</div>
        </div>
        @endif
        <button type="submit" class="w-full py-3 bg-gradient-to-r from-blue-500 to-blue-700 dark:from-blue-800 dark:to-blue-600 text-white font-bold rounded-xl shadow-lg hover:scale-105 hover:from-blue-600 hover:to-blue-800 transition-all duration-300">Firmar PDF</button>
    </form>
