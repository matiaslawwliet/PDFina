<div class="max-w-xl mx-auto p-6 bg-white/80 dark:bg-zinc-900/80 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 backdrop-blur-xl animate-fade-in"
    x-data
    x-init="$wire.userTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone">
    ...
    <form wire:submit.prevent="sellar" class="space-y-6">
        <div>
        ...
        <div>
            <label class="block text-sm font-medium mb-1">Imagen del Sello (Formato PNG/JPG)</label>
            <input type="file" wire:model="sello" accept="image/png,image/jpeg" class="block w-full text-sm text-gray-700 dark:text-gray-200 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-100 file:text-blue-700 dark:file:bg-blue-900 dark:file:text-blue-200 hover:file:bg-blue-200 dark:hover:file:bg-blue-800 transition" />
            @error('sello')
                <div class="text-red-500 text-sm animate-shake">{{ $message }}</div>
            @enderror
        </div>
        ...
        @if($previewImagePath)
        <div class="mb-4">
            <label class="block text-sm font-medium mb-1">Selecciona la posición del sello en la cuadrícula:</label>
            ...
            <div class="text-xs text-gray-500 mt-2">Se agregará automáticamente la fecha y hora actual junto al sello. El documento será protegido contra edición.</div>
        </div>
        @endif
        <button type="submit" class="w-full py-3 bg-gradient-to-r from-blue-500 to-blue-700 dark:from-blue-800 dark:to-blue-600 text-white font-bold rounded-xl shadow-lg hover:scale-105 hover:from-blue-600 hover:to-blue-800 transition-all duration-300">Sellar PDF</button>
    </form>
