<div class="max-w-xl mx-auto p-6 bg-white/80 dark:bg-zinc-900/80 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 backdrop-blur-xl animate-fade-in">
    <h2 class="text-3xl font-extrabold mb-6 text-blue-700 dark:text-blue-300 tracking-tight flex items-center gap-2">
        ...
        DNI (Frente y Dorso) a PDF
    </h2>

    <form wire:submit.prevent="dniAPdf" class="space-y-6">
        <label class="block text-sm font-medium mb-1">Frente del DNI (JPG)</label>
        <input type="file" wire:model="front" accept=".jpg,.jpeg,image/jpeg" class="block w-full text-sm text-gray-700 dark:text-gray-200 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-100 file:text-blue-700 dark:file:bg-blue-900 dark:file:text-blue-200 hover:file:bg-blue-200 dark:hover:file:bg-blue-800 transition" />
        @error('front')
        ...
        <div class="mt-2">
            @if ($front)
                <img id="preview-front" src="{{ $front->temporaryUrl() }}" class="mx-auto max-h-48 object-contain" />
            @endif
        </div>

        <label class="block text-sm font-medium mb-1">Dorso del DNI (JPG)</label>
        <input type="file" wire:model="back" accept=".jpg,.jpeg,image/jpeg" class="block w-full text-sm text-gray-700 dark:text-gray-200 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-100 file:text-blue-700 dark:file:bg-blue-900 dark:file:text-blue-200 hover:file:bg-blue-200 dark:hover:file:bg-blue-800 transition" />
        @error('back')
            <div class="text-red-500 text-sm animate-shake">{{ $message }}</div>
        @enderror
        ...
        <div class="mt-2">
            @if ($back)
                <img id="preview-back" src="{{ $back->temporaryUrl() }}" class="mx-auto max-h-48 object-contain" />
            @endif
        </div>

        <button type="submit" class="w-full py-3 bg-gradient-to-r from-blue-500 to-blue-700 dark:from-blue-800 dark:to-blue-600 text-white font-bold rounded-xl shadow-lg hover:scale-105 hover:from-blue-600 hover:to-blue-800 transition-all duration-300">Generar PDF</button>
    </form>
